...
def simultaneous_decoding(funcs, agent, options,
    ...
    # first_ctx = h_pipe['ctx']
    #print h_pipe['ctx'][1].shape
    ...
    while live_k > 0:
        ...
        #try:
        #    if np.all(first_ctx[:, 1, :] == h_pipe['ctx'][:, 1, :]):
        #        print "same!", str(step)
        #    else:
        #        print "changed", str(step)
        #        print first_ctx[:, 1, :].shape
        #        print h_pipe['ctx'][:, 1, :].shape
        ...
        # print np.array(n_pipe['action']).shape
        # ------------------------------------------------------------------
        ...
        #if np.all(first_ctx==n_pipe['ctx']):
        #    print "Same!"
        #else:
        #    print "Changed!"

        #print n_pipe['ctx'].shape
        # make it numpy array
